"""Gera docs/execucao/01-run-e-ids.md - rastreabilidade da fase de execucao."""
import json
from pathlib import Path

IDS = Path("docs/execucao/_ids")
RUN = (IDS / "run_id.txt").read_text(encoding="utf-8").strip()
run_json = json.loads((IDS / "run.json").read_text(encoding="utf-8"))
criado = run_json["result"]["run"]["created_at"]
...
for f, dono, card, tid, did, onda in FRENTES:
    linhas.append(f"| {f} | {dono} | {card} | `{tid}` | `{did}` | {onda} |")
...
Path("docs/execucao/01-run-e-ids.md").write_text("\n".join(linhas) + "\n", encoding="utf-8")
print("\n".join(linhas[:34]))
print(f"\n[{len(linhas)} linhas gravadas em docs/execucao/01-run-e-ids.md]")
